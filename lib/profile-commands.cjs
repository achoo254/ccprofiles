...
/** Check if a profile has credentials file */
function profileHasCreds(name) {
  return fs.existsSync(path.join(PROFILES_DIR, name, CREDS_FILE));
}
...
/**
 * Get usage limits — cache-first from hook cache, fallback to API call.
 * Returns { five_hour, seven_day, seven_day_sonnet } or null.
 * Each bucket: { utilization: number, resets_at: string }
 */
async function getUsageLimits() {
  // 1. Try reading hook cache (fast, no network)
  const cacheFile = path.join(os.tmpdir(), 'ck-usage-limits-cache.json');
  try {
    const cache = JSON.parse(fs.readFileSync(cacheFile, 'utf8'));
    if (cache.status === 'available' && cache.data && (Date.now() - cache.timestamp) < 120000) {
      return cache.data;
    }
  } catch {}

  // 2. Fallback: call API directly with OAuth token
  const creds = readJson(path.join(CLAUDE_DIR, CREDS_FILE));
  const token = creds?.claudeAiOauth?.accessToken;
  if (!token) return null;

  return fetchUsageWithToken(token);
}

/** Fetch usage from Anthropic API with a specific OAuth token */
...
/** Get usage for a specific profile by reading its saved credentials */
async function getProfileUsage(profileName) {
  const creds = readJson(path.join(PROFILES_DIR, profileName, CREDS_FILE));
  const token = creds?.claudeAiOauth?.accessToken;
  return fetchUsageWithToken(token);
}
...
function cmdList() {
  ...
  const entries = fs.readdirSync(PROFILES_DIR, { withFileTypes: true })
    ...
    .map(e => {
      ...
      return {
        ...
        hasCreds: profileHasCreds(e.name)
      };
    });
  ...
  for (const e of entries) {
    ...
    const warn = e.hasCreds ? '' : ' ⚠ no credentials';
    console.log(`  ${e.name.padEnd(nameW)}  ${e.email.padEnd(emailW)}  ${e.subscription.padEnd(8)}  ${marker}${warn}`);
  }

  // Show re-save hint if any profile missing credentials
  const noCreds = entries.filter(e => !e.hasCreds);
  if (noCreds.length) {
    console.log(`\n  ⚠ ${noCreds.length} profile(s) missing credentials. Fix: login & run "ccprofiles save <name>"`);
  }
  console.log('');
}

/** Format usage bucket for display */
function formatUsageBucket(label, bucket) {
  if (!bucket) return null;
  const pct = bucket.utilization;
  const warn = pct >= 80 ? ' ⚠' : '';
  const resets = bucket.resets_at ? ` resets ${new Date(bucket.resets_at).toLocaleTimeString()}` : '';
  return `  ${label.padEnd(12)} ${String(pct).padStart(3)}% used${warn}${resets}`;
}

/** Show current profile status with extended info + usage */
async function cmdStatus() {
  ...
  // Usage limits (cache-first, fallback API)
  const usage = await getUsageLimits();
  if (usage) {
    console.log('  Usage:');
    const lines = [
      formatUsageBucket('5h session', usage.five_hour),
      formatUsageBucket('7d all', usage.seven_day),
      formatUsageBucket('7d sonnet', usage.seven_day_sonnet),
    ].filter(Boolean);
...
module.exports = {
  ...
  getCredentialExtras, getConfiguredModel, getUsageLimits, getProfileUsage,
  profileHasCreds
};
