...
<header>
  ...
  <div style="display:flex;align-items:center;gap:12px">
    <span id="ws-status" class="ws-status ws-err">&bull; connecting...</span>
...
.ws-status{font-size:.75rem;padding:2px 8px;border-radius:8px}
.ws-ok{color:#34c759}
.ws-err{color:var(--danger)}
...
let ws=null;let wsRetries=0;let wsConnected=false;
...
function updateWsStatus(connected){
  ...
  if(!el)return;
  if(connected){el.className='ws-status ws-ok';el.textContent='\\u25cf connected'}
  else{el.className='ws-status ws-err';el.textContent='\\u25cb reconnecting...'}
}
...
function connectWS(){
  ...
  ws.onopen=()=>{wsRetries=0;updateWsStatus(true)};
  ws.onmessage=(e)=>{
...
const pendingChecks=new Map(); // name → timeoutId for HTTP fallback

function applyCheckResult(name,msg){
  const card=findCard(name);
  if(!card)return;
  // Clear fallback timeout
  if(pendingChecks.has(name)){clearTimeout(pendingChecks.get(name));pendingChecks.delete(name)}
  ...
  if(msg.valid===true){
    ...
    // null = non-active, can't check remotely
    st.className='card-status status-unknown';
    st.textContent='\\u2014 '+msg.email+' ['+msg.subscriptionType+'] (not active)';
  }
...
function renderCard(p){
  ...
  return '<div class="card'+(isActive?' active':'')+'" data-name="'+n+'">'
    ...
    +'<div class="card-meta">'
    +'<span class="badge badge-sub">'+esc(p.subscriptionType)+'</span>'
    +(p.rateLimitTier?' <span class="badge badge-sub">'+esc(p.rateLimitTier)+'</span>':'')
    +(p.model?' <span class="badge badge-sub">model: '+esc(p.model)+'</span>':'')
    +'</div>'
...
function handleCheck(name){
  ...
  // HTTP fallback: if WS doesn't deliver result in 8s, poll server
  if(pendingChecks.has(name))clearTimeout(pendingChecks.get(name));
  pendingChecks.set(name,setTimeout(async()=>{
    pendingChecks.delete(name);
    const r=await api('GET','/api/check-result/'+encodeURIComponent(name));
    if(r.ok&&r.data){applyCheckResult(name,r.data)}
    else{
      // Still no result — show timeout error
      const c=findCard(name);
      if(c){
        const s=c.querySelector('.card-status');
        s.className='card-status status-invalid';
        s.textContent='\\u2717 check timed out';
        enableCardButtons(c);
      }
    }
  },8000));
