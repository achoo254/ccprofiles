...
#usage-bar{display:flex;gap:16px;margin-bottom:16px;flex-wrap:wrap}
.usage-item{flex:1;min-width:180px;background:var(--card);border:1px solid var(--border);border-radius:var(--radius);padding:12px 16px}
...
function renderUsage(data){
  ...
  const buckets=[
    {key:'five_hour',label:'5h session'},
    {key:'seven_day',label:'7d all models'},
  ...
  bar.innerHTML=buckets.map(b=>{
    ...
    const resets=d.resets_at?'resets '+new Date(d.resets_at).toLocaleTimeString():'';
    return '<div class="usage-item"><div class="usage-label">'+b.label+'</div>'
      ...
      +'<div class="usage-pct">'+pct+'% <span style="font-weight:400;font-size:.75rem;color:var(--text2)">'+resets+'</span></div></div>';
  }).join('');
}
...
async function reloadAll(){
  ...
  api('GET','/api/check-all').then(()=>{
    // Poll active profile check result after 3s
    const active=document.querySelector('.card.active');
    if(active)pollCheckResult(active.dataset.name,0);
  });
  api('GET','/api/usage').then(r=>{if(r.ok)renderUsage(r.data)});
}
