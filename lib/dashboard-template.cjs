 ...
 * HTTP-only (no WebSocket) — uses polling for check results.
 */
...
<header>
  ...
  <div style="display:flex;align-items:center;gap:8px">
    <button class="btn btn-secondary" onclick="reloadAll()">&#x21bb; Reload</button>
...
.badge{font-size:.65rem;padding:2px 6px;border-radius:8px;font-weight:500}
.badge-active{background:var(--accent);color:#fff}
.badge-sub{background:var(--border);color:var(--text2)}
.card-email{color:var(--text2);font-size:.8rem;margin-bottom:4px}
.card-meta{display:flex;flex-wrap:wrap;gap:4px;margin-bottom:4px}
.card-saved{color:var(--text2);font-size:.7rem;margin-bottom:8px}
.card-status{font-size:.8rem;margin-bottom:12px;min-height:1.2em}
...
.card-usage{display:flex;gap:6px;margin-bottom:10px;flex-wrap:wrap}
.card-usage .usage-item{flex:1;min-width:0;padding:6px 8px;border-radius:6px;background:var(--bg)}
.card-usage .usage-label{font-size:.6rem;margin-bottom:3px}
.card-usage .usage-track{height:4px}
.card-usage .usage-pct{font-size:.7rem;margin-top:2px}
.card-actions{display:flex;gap:6px;flex-wrap:wrap}
.btn{border:none;padding:6px 14px;border-radius:8px;font-size:.8rem;font-weight:500;cursor:pointer;transition:background .2s,opacity .2s}
...
.usage-item{flex:1;min-width:0;background:var(--card);border:1px solid var(--border);border-radius:var(--radius);padding:8px 10px}
.usage-label{font-size:.75rem;color:var(--text2);margin-bottom:6px}
...
.usage-pct{font-size:.85rem;font-weight:600;margin-top:4px}
`;
}
...
function applyCheckResult(name,msg){
  ...
  if(msg.valid===true){
    ...
    st.className='card-status status-unknown';
    st.textContent='\\u2014 not checked (switch to check)';
  }
...
function renderCard(p){
  ...
  const badges=[];
  badges.push('<span class="badge badge-sub">'+esc(p.subscriptionType)+'</span>');
  if(p.rateLimitTier)badges.push('<span class="badge badge-sub">'+esc(p.rateLimitTier)+'</span>');
  if(p.model)badges.push('<span class="badge badge-sub">'+esc(p.model)+'</span>');

  return '<div class="card'+(isActive?' active':'')+'" data-name="'+n+'">'
    +'<div class="card-header"><span class="card-name">'+esc(p.email)+'</span>'
    +(isActive?'<span class="badge badge-active">active</span>':'')
    +'</div>'
    +'<div class="card-meta">'+badges.join(' ')+'</div>'
    +'<div class="card-saved">'+saved+'</div>'
    +'<div class="card-status status-checking"><span class="spinner"></span>checking...</div>'
    +'<div class="card-usage" id="usage-'+n+'"></div>'
...
async function handleSwitch(name){
  ...
  else{showToast('Switched to "'+name+'"');reloadAll()}
}
...
async function handleDelete(name){
  ...
  if(!r.ok)showToast(r.error,'error');
  else{showToast('Deleted "'+name+'"');reloadAll()}
}
...
async function handleCheck(name){
  ...
  // Trigger check then poll for result
  await api('GET','/api/check/'+encodeURIComponent(name));
  pollCheckResult(name,0);
}

async function pollCheckResult(name,attempt){
  if(attempt>5)return applyCheckResult(name,{valid:false});
  await new Promise(r=>setTimeout(r,2000));
  const r=await api('GET','/api/check-result/'+encodeURIComponent(name));
  if(r.ok&&r.data&&r.data.valid!==undefined){
    applyCheckResult(name,r.data);
  }else{
    pollCheckResult(name,attempt+1);
  }
}
...
async function handleSave(){
  ...
  if(!r.ok)showToast(r.error,'error');
  else{showToast('Saved "'+r.data.name+'"');reloadAll()}
}
...
function renderCardUsage(name,data){
  ...
  el.innerHTML=buckets.map(b=>{
    ...
    const color=pct>=80?'var(--danger)':pct>=50?'#ff9f0a':'#34c759';
    return '<div class="usage-item"><div class="usage-label">'+b.label+'</div>'
      +'<div class="usage-track"><div class="usage-fill" style="width:'+pct+'%;background:'+color+'"></div></div>'
      +'<div class="usage-pct">'+pct+'%</div></div>';
  }).join('');
}
...
async function loadAllUsage(){
  ...
  cards.forEach(card=>{
    ...
    api('GET','/api/usage/'+encodeURIComponent(name)).then(r=>{
      if(r.ok&&r.data)renderCardUsage(name,r.data);
    });
  });
}
...
async function reloadAll(){
  await loadProfiles();
  api('GET','/api/check-all').then(()=>{
    const active=document.querySelector('.card.active');
    if(active)pollCheckResult(active.dataset.name,0);
  });
  loadAllUsage();
}
