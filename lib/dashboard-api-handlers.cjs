...
const { getActiveProfile, setActiveProfile, backupBase, getAuthStatus, getCredentialExtras, getConfiguredModel, getUsageLimits, getProfileUsage } = require('./profile-commands.cjs');
...
/** Run `claude auth status` via spawn (non-blocking). Caches result for polling. */
...
/** GET /api/check/:name — async token validation, poll /api/check-result/:name for result */
...
/** GET /api/check-result/:name — return cached check result */
...
/** GET /api/usage — fetch usage limits for active profile (cache-first, API fallback) */
async function handleUsage(res, json) {
  try {
    const data = await getUsageLimits();
    json(res, 200, { ok: true, data: data || null });
  } catch (err) {
    json(res, 500, { ok: false, error: err.message });
  }
}

/** GET /api/usage/:name — fetch usage for a specific profile using its saved token */
async function handleProfileUsage(res, name, json) {
  const profileDir = path.join(PROFILES_DIR, name);
  if (!fs.existsSync(profileDir)) {
    return json(res, 404, { ok: false, error: `Profile "${name}" not found` });
  }
  try {
    const data = await getProfileUsage(name);
    json(res, 200, { ok: true, data: data || null });
  } catch (err) {
    json(res, 500, { ok: false, error: err.message });
  }
